...
"""
    DART 2021 data (simulated) extraction
"""
...
def get_old_images(path, polymers, cover_percent, resample_method):
    imagery = []
    for polymer in polymers:
            ...
            if cover == polymer:
                ...
                for percent in percents:
                    ...
                    if files_list:
                        ...
                        current_image = Image_old(directory, files_list, percent, bands, resample_method)   #AQUI - DAR OPCAO DE REAMOSTRAGEM
                        imagery.append(current_image)               
    return imagery


def build_old_dataset(image_collection):
    bands = []
...
class Image_old:
    ...
    def setPixels(self):
        ...
        files = dict()
        for file_name in self.getFileNames():
            current_file = old_Band(self.getPath(), file_name)
    ...
    def mountMultiband(self, files):
        ...
        for n_line in range(self.getXSize()):
            for n_col in range(self.getYSize()):
                current_pixel = old_Pixel(n_line, n_col, self.getPath())
                for key in files:
...
"""
    End of DART 2021 data (simulated) extraction
"""



"""
    DART 2023 data (simulated) extraction
"""
